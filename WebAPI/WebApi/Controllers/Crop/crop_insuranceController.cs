...
namespace WebApi.Controllers.Crop
{
    ...
    public class crop_insuranceController : ApiController
    {
        ...
        public IHttpActionResult Putcrop_insurance(int id, crop_insurance crop_insurance)
        {
        ...
        public IHttpActionResult Postcrop_insurance(crop_insurance crop_insurance)
        {
            ...
            {
                if (crop_insuranceExists(crop_insurance.cropid))
                {
                    return Conflict();
                }
                else
            ...
            return CreatedAtRoute("DefaultApi", new { id = crop_insurance.cropid }, crop_insurance);
        }

        // DELETE: api/crop_insurance/5
        [ResponseType(typeof(crop_insurance))]
        public IHttpActionResult Deletecrop_insurance(int id)
        {
            crop_insurance crop_insurance = db.crop_insurance.Find(id);
            if (crop_insurance == null)
            {
        ...
        private bool crop_insuranceExists(int id)
        {
            return db.crop_insurance.Count(e => e.cropid == id) > 0;
        }
    }
}